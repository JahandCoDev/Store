...
type QuotePreviewMode = "customer" | "admin";
...
function formatQuoteAddOns(addOns: string[]) {
  return addOns.length ? addOns.join(", ") : "None selected";
}

function buildSamplePayload(overrides?: Partial<QuoteMailPayload>): QuoteMailPayload {
  return {
    id: "preview-quote-001",
    name: "Jordan Carter",
    email: "jordan@example.com",
    phone: "+1 (407) 555-0183",
    selectedPlan: "Small Business",
    projectType: "Business Website",
    budgetRange: "$500-$900",
    timeline: "2-3 weeks",
    addOns: ["AI Integration", "Custom Email Setup"],
    details:
      "I need a clean site for a local service business with a strong homepage, quote form, and simple admin updates for content.",
    createdAt: new Date(),
    ...overrides,
  };
}
...
function getSignatureAttachment(): MailAttachment | null {
  ...
  const candidates = [
    configuredPath,
    path.resolve(process.cwd(), "../storefront/public/signature.PNG"),
    path.resolve(process.cwd(), "../storefront/public/signature.png"),
        ...
        <div style="padding:28px 32px;">
          <div style="margin-bottom:24px;padding:20px 22px;border-radius:20px;background:linear-gradient(180deg,rgba(15,23,42,0.92),rgba(15,23,42,0.68));border:1px solid rgba(148,163,184,0.12);">
            ...
            <table role="presentation" width="100%" style="border-collapse:collapse;">
              <tr><td style="padding:0 0 10px;color:#94a3b8;font-size:14px;">Submitted</td><td style="padding:0 0 10px;color:#f8fafc;font-size:14px;text-align:right;">${escapeHtml(submittedAt)}</td></tr>
              <tr><td style="padding:0 0 10px;color:#94a3b8;font-size:14px;">Project type</td><td style="padding:0 0 10px;color:#f8fafc;font-size:14px;text-align:right;">${projectType}</td></tr>
              <tr><td style="padding:0 0 10px;color:#94a3b8;font-size:14px;">Budget</td><td style="padding:0 0 10px;color:#f8fafc;font-size:14px;text-align:right;">${budgetRange}</td></tr>
              <tr><td style="padding:0 0 10px;color:#94a3b8;font-size:14px;">Timeline</td><td style="padding:0 0 10px;color:#f8fafc;font-size:14px;text-align:right;">${timeline}</td></tr>
              <tr><td style="padding:0 0 10px;color:#94a3b8;font-size:14px;">Selected plan</td><td style="padding:0 0 10px;color:#f8fafc;font-size:14px;text-align:right;">${selectedPlan}</td></tr>
              <tr><td style="padding:0;color:#94a3b8;font-size:14px;">Phone</td><td style="padding:0;color:#f8fafc;font-size:14px;text-align:right;">${phone}</td></tr>
            </table>
          </div>
...
export function getQuoteEmailPreview(options?: {
  mode?: QuotePreviewMode;
  payload?: Partial<QuoteMailPayload>;
}) {
  const payload = buildSamplePayload(options?.payload);
  ...
  if (mode === "admin") {
    return {
      subject: `Preview: New JahandCo quote request from ${payload.name}`,
      text: [
        `Submission ID: ${payload.id}`,
        `Name: ${payload.name}`,
        `Email: ${payload.email}`,
  ...
  return {
    subject: "Preview: We received your JahandCo project request",
    text: [
      `Hi ${payload.name},`,
      "",
      "Thanks for reaching out to JahandCo.",
      "I received your project request and will review it personally before following up with a clear next step.",
      "",
      `Submitted: ${submittedAt}`,
      `Project type: ${payload.projectType}`,
...
export async function sendQuotePreviewEmail(options: {
  to: string;
  mode?: QuotePreviewMode;
  payload?: Partial<QuoteMailPayload>;
}) {
